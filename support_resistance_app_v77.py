...
def choose_tick_positions(index: pd.DatetimeIndex, interval: str, period: str) -> Tuple[List[pd.Timestamp], List[str]]:
    ...
    def add_nearest(ts: pd.Timestamp, label: str):
        ...
        real_ts = index[pos]
        if real_ts not in positions:
            positions.append(real_ts)
            labels.append(label)
    ...
    if interval in {"1m", "5m", "15m"}:
        if period == "1d" or len(unique_days) == 1:
            ...
            for t in preferred:
                add_nearest(pd.Timestamp(f"{base_day.date()} {t}"), t)
        elif len(unique_days) <= 3:
            preferred = ["09:00", "11:00", "13:30", "15:00"]
            for day in unique_days:
                for t in preferred:
                    add_nearest(pd.Timestamp(f"{day.date()} {t}"), f"{day.strftime('%d %b')}<br>{t}")
        else:
            step = max(1, len(unique_days) // 8)
            for day in unique_days[::step]:
                pos = index[index.normalize() == day][0]
        ...
        if len(unique_days) <= 2:
            ...
            for i in range(0, len(index), step):
                ...
                labels.append(index[i].strftime("%d %b<br>%H:%M"))
    elif interval == "4h":
    ...
    if len(positions) == 0:
        ...
        for i in range(0, len(index), step):
            positions.append(index[i])
            labels.append(index[i].strftime("%d %b<br>%H:%M") if interval != "1d" else index[i].strftime("%d %b %Y"))
...
def add_sr_lines(fig: go.Figure, df: pd.DataFrame, levels: Dict[str, Optional[LevelCluster]], y_min: float, y_max: float):
    ...
    for (cluster, tag), label_y in zip(line_items, placed_levels):
        start_x = df.index[max(0, cluster.anchor_idx)]
        fig.add_shape(
            type="line",
            x0=start_x,
            x1=last_x,
...
def add_ma_and_close_labels(fig: go.Figure, df: pd.DataFrame):
    last_x = df.index[-1]
    last_close = float(df["Close"].iloc[-1])
    ...
    if not ma_series.empty:
        ...
        fig.add_annotation(
            ...
            xref="x",
            yref="y",
            text=f"MA10 {price_step_formatter(last_ma)}",
            showarrow=False,
            xanchor="left",
            yanchor="middle",
            xshift=56,
...
def add_pattern_markers(fig: go.Figure, df: pd.DataFrame, signals: List[PatternSignal], y_pad: float):
    ...
    for sig in signals:
        ...
        xs.append(df.index[i])
        ys.append(high + y_pad)
        texts.append(f"{sig.name}<br>{sig.probability * 100:.0f}% {sig.bias}")
...
def build_chart(df: pd.DataFrame, interval: str, period: str) -> Tuple[go.Figure, Dict[str, Optional[LevelCluster]], str, List[PatternSignal]]:
    ...
    fig.add_trace(
        go.Bar(x=df.index, y=df["Volume"], marker_color=bar_colors, name="Volume", opacity=0.85),
        row=2,
        col=1,
    )
    fig.add_trace(
        go.Scatter(
            ...
            y=df["VMA20"],
            mode="lines",
            line=dict(color=MA_VOL_COLOR, width=1.6),
            name="VMA20",
        ),
        row=2,
        col=1,
    ...
    add_ma_and_close_labels(fig, df)
    ...
    fig.update_yaxes(showgrid=True, gridcolor="#DADADA", side="right", row=1, col=1, title_text="Price")
    fig.update_yaxes(showgrid=True, gridcolor="#DADADA", side="right", row=2, col=1, title_text="Volume")
    ...
    fig.update_layout(
        ...
        paper_bgcolor="white",
        margin=dict(l=16, r=180, t=18, b=18),
        height=780,
